...
interface AnalyzerAppProps {
  onGeoJsonGenerated: (data: GeoJsonOutput | null) => void;
}
...
const checkUrlIsRssCompliant = async (url: string): Promise<boolean> => {
  ...
  try {
    ...
    if (headResponse.ok && isValidRssContentType(headResponse.headers.get('Content-Type'))) return true;

    const getResponse = await fetch(fetchUrl);
    if (!getResponse.ok) return false;
    if (isValidRssContentType(getResponse.headers.get('Content-Type'))) return true;
    
    const text = await getResponse.text();
    const parser = new DOMParser();
    const xmlDoc = parser.parseFromString(text, "application/xml");
    return xmlDoc.getElementsByTagName("parsererror").length === 0 && 
           (xmlDoc.getElementsByTagName("rss").length > 0 || xmlDoc.getElementsByTagName("feed").length > 0);
  } catch (error) {
...
const makeGeoJson = (articles: AnalyzedArticle[]): GeoJsonOutput => {
  const features: GeoJsonFeature[] = articles
    ...
    .map(article => ({
      ...
      properties: { 
        titre: article.titre, // Use AI-derived/merged title
        categorie: article.categorie,
        importance: article.importance,
        lien: article.lien, // Use AI-derived/merged link
        localisation: article.localisation,
        date: article.date, // Use AI-derived/merged date
        description: article.description, // Use AI-derived/merged description
        imageUrl: article.imageUrl, // Use AI-derived/merged imageUrl
      },
    }));
...
const formatTimeMs = (milliseconds: number): string => {
  ...
  if (minutes > 0) return `Est. ~${minutes} min ${seconds < 10 ? '0' : ''}${seconds} sec remaining`;
  if (totalSeconds > 0) return `Est. ~${totalSeconds} sec remaining`;
...
const AnalyzerAppComponent: React.FC<AnalyzerAppProps> = ({ onGeoJsonGenerated }) => {
  ...
  const [geoJsonDataState, setGeoJsonDataState] = useState<GeoJsonOutput | null>(null);
  ...
  const performArticleAnalysisBatch = useCallback(async (
    ...
    if (!apiKeyExists || articlesToAnalyze.length === 0) {
       if (!apiKeyExists) console.error("Analysis skipped: OpenAI API Key missing for batch.");
      return results;
    }
    ...
    const defaultFailedAnalysisData: Partial<GeminiAnalysisResult> = { 
        importance: 0.0, categorie: 'flash', localisation: 'Analyse échouée (par défaut)',
        latitude: null, longitude: null,
        titre: "", lien: "", date: new Date().toISOString().split('T')[0], description: "", imageUrl: ""
    };
    ...
    for (let i = 0; i < batchTotal; i++) {
      ...
      if (i > 0) { 
          ...
          estimatedRemainingStr = formatTimeMs((batchTotal - i) * avgTimePerArticleMs);
      }
      onProgressUpdate({ current: i + 1, total: batchTotal, estimatedRemaining: estimatedRemainingStr, currentTitle: article.title.substring(0, 50) + (article.title.length > 50 ? "..." : "") });
      
      try {
        const aiAnalysisResult = await analyzeArticleContent(article); 
        if (aiAnalysisResult) {
          results.push({
            ...article, 
            titre: aiAnalysisResult.titre || article.title, 
            categorie: aiAnalysisResult.categorie || 'flash',
            importance: aiAnalysisResult.importance ?? 0.0,
            lien: aiAnalysisResult.lien || article.link,
            localisation: aiAnalysisResult.localisation || 'N/A',
            date: aiAnalysisResult.date || article.pubDate || new Date().toISOString().split('T')[0], 
            description: aiAnalysisResult.description || article.description || "Description non disponible.", 
            imageUrl: aiAnalysisResult.imageUrl || article.imageUrl || "", 
            latitude: aiAnalysisResult.latitude,
            longitude: aiAnalysisResult.longitude,
          } as AnalyzedArticle); 
        } else {
          results.push({ 
            ...article, 
            ...defaultFailedAnalysisData, 
            titre: article.title, 
            lien: article.link, 
            date: article.pubDate || defaultFailedAnalysisData.date!, 
            description: article.description || defaultFailedAnalysisData.description!, 
            imageUrl: article.imageUrl || defaultFailedAnalysisData.imageUrl! 
          } as AnalyzedArticle);
        }
      } catch (err) {
        console.error(`Failed to analyze article ${article.id} in batch with OpenAI:`, err);
        results.push({ 
            ...article, 
            ...defaultFailedAnalysisData, 
            titre: article.title, 
            ...
            description: article.description || defaultFailedAnalysisData.description!,
            imageUrl: article.imageUrl || defaultFailedAnalysisData.imageUrl!,
            localisation: 'Erreur d\'analyse batch (OpenAI)' 
        } as AnalyzedArticle);
      }
    }
  ...
  const handleAddRssSource = async () => {
    ...
    if (!trimmedName || !trimmedUrl) { setError("Source Name and URL cannot be empty."); return; }
    if (!/^https?:\/\//i.test(trimmedUrl)) { setError("Please enter a valid URL."); return; }
    if (allRssSources.find(s => s.url === trimmedUrl)) { setError("This RSS feed URL already exists."); return; }
    ...
    if (isValidFeed) {
      ...
      setNewRssSourceName(""); setNewRssSourceUrl("");
      setNotification(`RSS source "${trimmedName}" added.`);
    } else {
      setError(`URL "${trimmedUrl}" does not appear to be a valid RSS/XML feed or could not be reached.`);
    }
  ...
  const handleFetchRss = useCallback(async () => {
    ...
    setError(null); setNotification(null);
    if (sourcesToFetch.length === 0) { setError("No RSS sources selected."); return; }

    setIsLoadingRss(true); setAutoRefreshLog([]); 
    try {
      const articles = await fetchAllRssFeeds(sourcesToFetch);
      setRawArticles(articles); setCurrentView(View.RAW_ARTICLES); 
      setAnalyzedArticles([]); 
      const newGeoJson = makeGeoJson([]); // Create empty GeoJSON
      setGeoJsonDataState(newGeoJson); 
      onGeoJsonGenerated(newGeoJson); // Pass empty GeoJSON so map doesn't use stale data
      setAnalysisBatchProgressData(null);
      setNotification(articles.length > 0 ? `${articles.length} articles fetched.` : "No articles found.");
    } catch (err) { setError("Failed to fetch RSS feeds."); } 
    finally { setIsLoadingRss(false); setLastRefreshTime(Date.now()); setAutoRefreshProgress(0); }
  }, [allRssSources, selectedRssSourceUrls, onGeoJsonGenerated]);

  const handleAnalyzeArticles = useCallback(async () => {
    setError(null); setNotification(null);
    if (!apiKeyExists) { setError("OpenAI API key is not configured. Analysis cannot proceed. Please set VITE_OPENAI_API_KEY in your .env.local file."); return; }
    if (articlesToProcessCount === 0) { setError("No articles to analyze."); return; }
    
    setIsLoadingAnalysis(true); setAutoRefreshLog([]);
    const articlesToAnalyzeSlice = rawArticles.slice(0, articlesToProcessCount);
    const results = await performArticleAnalysisBatch(articlesToAnalyzeSlice, setAnalysisBatchProgressData);
    
    setAnalyzedArticles(results);
    const newGeoJson = makeGeoJson(results);
    setGeoJsonDataState(newGeoJson);
    onGeoJsonGenerated(newGeoJson); 
    setCurrentView(View.ANALYZED_ARTICLES);
    setIsLoadingAnalysis(false);

    if (results.length > 0) setNotification(`${results.length} articles analyzed with OpenAI.`);
    else if (articlesToProcessCount > 0) setError("OpenAI Analysis completed, but no data was successfully analyzed or geolocated.");

  }, [rawArticles, articlesToProcessCount, apiKeyExists, performArticleAnalysisBatch, onGeoJsonGenerated]); 


  const handleClearData = useCallback(() => {
    setRawArticles([]); setAnalyzedArticles([]);
    const emptyGeoJson = makeGeoJson([]);
    setGeoJsonDataState(emptyGeoJson); 
    onGeoJsonGenerated(emptyGeoJson);
    setError(null); setNotification(null);
    setAnalysisBatchProgressData(null); setAutoRefreshLog([]);
    setCurrentView(View.RAW_ARTICLES);
    setLastRefreshTime(Date.now()); setAutoRefreshProgress(0);
  }, [onGeoJsonGenerated]);

  const handleAutoRefresh = useCallback(async () => {
    if (isAutoRefreshing || !apiKeyExists || selectedRssSourceUrls.length === 0) {
      if (!apiKeyExists) setAutoRefreshLog(prev => prev.length > 0 && prev[0].startsWith("Auto-refresh:") ? prev : ["Auto-refresh: Paused (OpenAI API key missing)."]);
    ...
    setIsAutoRefreshing(true); setNotification(null); setAnalysisBatchProgressData(null); 
    ...
    try {
      await fetchAllRssFeeds(
        ...
        (sourceName, articlesFromSource, fetchError) => {
          ...
          tempLog.push(`- ${sourceName}: Fetched ${articlesFromSource.length}, ${newFromThisSource.length} new.${fetchError ? ` Error: ${fetchError.substring(0,30)}` : ''}`);
          newFromThisSource.forEach(nfa => { if (!allNewlyFetchedArticlesForAnalysis.find(a => a.id === nfa.id)) allNewlyFetchedArticlesForAnalysis.push(nfa); });
          setAutoRefreshLog([...tempLog]);
        }
      );
      
      if (allNewlyFetchedArticlesForAnalysis.length > 0) {
           setRawArticles(prevRaw => [...prevRaw, ...allNewlyFetchedArticlesForAnalysis.filter(n => !new Set(prevRaw.map(r => r.id)).has(n.id))]);
      }

      if (allNewlyFetchedArticlesForAnalysis.length === 0) {
        tempLog.push("Auto-refresh: No new unique articles found.");
      } else {
        tempLog.push(`Auto-refresh: Total ${allNewlyFetchedArticlesForAnalysis.length} new articles. Analyzing with OpenAI...`);
        const analysisResults = await performArticleAnalysisBatch(allNewlyFetchedArticlesForAnalysis, setAnalysisBatchProgressData);
        if (analysisResults.length > 0) {
          const newCombinedAnalyzedArticles = [...analyzedArticles, ...analysisResults];
          setPreviousGeoJsonDataForComparison(geoJsonDataState);
          const newGeoJson = makeGeoJson(newCombinedAnalyzedArticles);
          setGeoJsonDataState(newGeoJson); onGeoJsonGenerated(newGeoJson);
          ...
          newFeaturesLinks.forEach(link => { if (!oldFeaturesLinks.has(link)) newlyAddedToGeoJsonCount++; });
          tempLog.push(`GeoJSON: ${newlyAddedToGeoJsonCount} new map entries. Total: ${newGeoJson.features.length}.`);
        } else {
          tempLog.push("Auto-refresh: OpenAI Analysis yielded no new results for GeoJSON.");
        }
      }
      tempLog.push("Auto-refresh: Cycle complete.");
      setAutoRefreshLog([...tempLog]);
    } catch (err) {
      tempLog.push("Auto-refresh: A critical error occurred."); setAutoRefreshLog([...tempLog]);
    } finally {
      setIsAutoRefreshing(false); setLastRefreshTime(Date.now()); setAutoRefreshProgress(0);
      setTimeout(() => { if(analysisBatchProgressData?.current === analysisBatchProgressData?.total) setAnalysisBatchProgressData(null); }, 4000); 
    }
  }, [isAutoRefreshing, apiKeyExists, selectedRssSourceUrls, allRssSources, rawArticles, analyzedArticles, geoJsonDataState, onGeoJsonGenerated, previousGeoJsonDataForComparison, performArticleAnalysisBatch]);
  ...
  useEffect(() => {
    ...
    if (apiKeyExists && selectedRssSourceUrls.length > 0 && !isAutoRefreshing && !(isLoadingAnalysis || isLoadingRss) ) {
      const updateTimer = () => {
        ...
        setAutoRefreshProgress(progress < 0 ? 0 : progress);
        if (elapsed >= AUTO_REFRESH_INTERVAL_MS) handleAutoRefresh();
      };
      progressIntervalId = window.setInterval(updateTimer, PROGRESS_UPDATE_INTERVAL_MS);
      updateTimer(); 
    } else { setAutoRefreshProgress(0); }
    return () => { if (progressIntervalId) clearInterval(progressIntervalId); };
  }, [apiKeyExists, selectedRssSourceUrls, lastRefreshTime, handleAutoRefresh, isAutoRefreshing, isLoadingAnalysis, isLoadingRss]);
  

  const renderView = () => {
    switch (currentView) {
      case View.RAW_ARTICLES: return <ArticleTable articles={rawArticles} isAnalyzed={false} />;
      case View.ANALYZED_ARTICLES: return <ArticleTable articles={analyzedArticles} isAnalyzed={true} />;
      case View.GEOJSON_OUTPUT: return <GeoJsonDisplay geoJsonData={geoJsonDataState} />;
      default: return <p className="text-center text-gray-400">Select a view.</p>;
    }
  };

  const TabButton: React.FC<TabButtonProps> = ({ view, label, icon, disabled }) => (
    <button onClick={() => { if (!disabled) setCurrentView(view); }}
    ...
    >
      {icon} <span className="ml-1 sm:ml-2">{label}</span>
    </button>
  );
  
  const hasDataToClear = rawArticles.length > 0 || analyzedArticles.length > 0 || geoJsonDataState !== null;
  const currentArticlesForManualAnalysis = numToAnalyze === 'all' ? rawArticles.length : Math.min(numToAnalyze, rawArticles.length);
  const analyzeButtonTextDisplay = isLoadingAnalysis ? 'Analyzing...' : `Analyze (${currentArticlesForManualAnalysis})`;

  return (
    <div className="min-h-full bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-gray-200 p-4 sm:p-6 md:p-8">
      <header className="mb-6 sm:mb-8 text-center">
        ...
        <p className="mt-2 text-md sm:text-lg text-gray-400">
          Fetch, Analyze (OpenAI), and Visualize News Articles
        </p>
        <div className="mt-3 space-y-1">
            { apiKeyExists && selectedRssSourceUrls.length > 0 && (
                 <div className="w-full max-w-md mx-auto bg-slate-700/80 rounded-full h-2.5 my-1 shadow-inner" title={`Next auto-refresh: ${Math.round(AUTO_REFRESH_INTERVAL_MS * (1 - autoRefreshProgress / 100) / 1000)}s`}>
                    <div className={`bg-sky-500 h-2.5 rounded-full transition-all duration-150 ease-linear ${isAutoRefreshing || isLoadingAnalysis ? 'animate-pulse' : ''}`}
                        style={{ width: `${(isAutoRefreshing || isLoadingAnalysis) ? 100 : autoRefreshProgress}%` }}
                        role="progressbar" aria-valuenow={(isAutoRefreshing || isLoadingAnalysis) ? 100 : autoRefreshProgress} aria-valuemin={0} aria-valuemax={100} aria-label="Time until next auto-refresh"
                    ></div>
                </div>
            )}
            ...
            <div className="p-2.5 bg-sky-700/50 text-sky-200 text-xs sm:text-sm rounded-md shadow-md max-w-2xl mx-auto">
                <div className="flex items-center font-semibold mb-1">
                    {(isAutoRefreshing && (!analysisBatchProgressData || analysisBatchProgressData.current < analysisBatchProgressData.total) ) ? 
                        <LoadingSpinner size="w-4 h-4" color="text-sky-300"/> : <RefreshIcon className="w-4 h-4 text-sky-300"/> }
                    <span className="ml-2">{autoRefreshLog[0]}</span>
                </div>
                {autoRefreshLog.slice(1).map((line, index) => (<p key={index} className="text-xs pl-6 leading-snug">{line}</p>))}
            </div>
            )}
        </div>
      </header>
      ...
      <div className="mb-6 p-4 sm:p-6 bg-slate-800 bg-opacity-80 backdrop-blur-md rounded-xl shadow-2xl">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto pr-2 mb-4 border border-slate-700 rounded-md p-3 bg-slate-900/50 custom-scrollbar scrollbar-thin">
            {allRssSources.length === 0 && <p className="text-sm text-gray-400">No RSS sources. Add one below.</p>}
            {allRssSources.map(source => (
              <div key={source.url} className="flex items-center justify-between bg-slate-700 p-2.5 rounded-md shadow">
                <div className="flex items-center flex-grow min-w-0">
                  <input type="checkbox" id={`rss-checkbox-${source.url}`} checked={selectedRssSourceUrls.includes(source.url)}
                    onChange={() => handleToggleRssSourceSelection(source.url)}
                    className="h-4 w-4 text-indigo-500 border-gray-500 rounded focus:ring-indigo-400 mr-3 shrink-0"
                    aria-labelledby={`rss-label-${source.url}`}
                    disabled={isAutoRefreshing || isLoadingRss || isLoadingAnalysis || isValidatingUrl} />
                  <label htmlFor={`rss-checkbox-${source.url}`} id={`rss-label-${source.url}`} className="text-sm text-gray-200 flex-grow cursor-pointer min-w-0">
                    <span className="font-medium block truncate" title={source.name}>{source.name}</span>
                    <span className="block text-xs text-gray-400 truncate" title={source.url}>{source.url}</span>
                  </label>
                </div>
                <button onClick={() => handleRemoveRssSource(source.url)}
                  className="p-1.5 text-red-400 hover:text-red-300 hover:bg-red-700/50 rounded-full transition-colors shrink-0 ml-2"
                  aria-label={`Remove ${source.name}`} title={`Remove ${source.name}`}
          ...
          <div className="flex flex-col sm:flex-row items-stretch gap-2 mt-3">
            <input type="text" placeholder="Source Name (e.g., My News)" value={newRssSourceName} onChange={(e) => setNewRssSourceName(e.target.value)}
              ...
              aria-label="New RSS Source Name" disabled={isLoadingRss || isLoadingAnalysis || isValidatingUrl || isAutoRefreshing} />
            <input type="url" placeholder="Source URL (https://...)" value={newRssSourceUrl} onChange={(e) => setNewRssSourceUrl(e.target.value)}
              className="flex-grow px-3 py-2 bg-slate-700 border border-slate-600 text-white rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm disabled:opacity-60"
              aria-label="New RSS Source URL" disabled={isLoadingRss || isLoadingAnalysis || isValidatingUrl || isAutoRefreshing} />
            <button onClick={handleAddRssSource}
              ...
              aria-label="Add new RSS source" title="Add new RSS source"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="space-y-2">
             <label htmlFor="numToAnalyzeSelect" className="block text-xs font-medium text-purple-200 mb-1">Articles for Manual Analysis:</label>
              <select id="numToAnalyzeSelect" value={String(numToAnalyze)}
                onChange={(e) => setNumToAnalyze(e.target.value === 'all' ? 'all' : parseInt(e.target.value, 10))}
                ...
                <option value="50">Top 50</option><option value="all">All ({rawArticles.length})</option>
              </select>
            <button onClick={handleAnalyzeArticles}
           ...
           <button onClick={handleClearData}
            ...
            <ClearIcon className="w-5 h-5"/> <span className="ml-2">Clear Data</span>
          </button>
        </div>
        ...
        {(isLoadingAnalysis || (isAutoRefreshing && analysisBatchProgressData)) && analysisBatchProgressData && analysisBatchProgressData.total > 0 && (
          <div role="status" aria-live="polite" className="mt-4 p-3 bg-slate-700 rounded-md shadow">
            <p className="text-sm text-yellow-300 mb-1 truncate">Analyzing: {analysisBatchProgressData.currentTitle || 'article...'} ({analysisBatchProgressData.current} of {analysisBatchProgressData.total})</p>
            <div className="w-full bg-slate-600 rounded-full h-2.5 mb-1">
              <div className="bg-yellow-400 h-2.5 rounded-full transition-width duration-150 ease-linear" style={{ width: `${(analysisBatchProgressData.current / analysisBatchProgressData.total) * 100}%` }}></div>
            </div>
            {analysisBatchProgressData.estimatedRemaining && (<p className="text-xs text-yellow-200 text-right">{analysisBatchProgressData.estimatedRemaining}</p>)}
          </div>
        )}

        {!apiKeyExists ? (
          <div role="alert" className="mt-4 p-3 bg-yellow-600 text-black rounded-md shadow-md text-sm">
            <strong>Warning:</strong> OpenAI API key not configured. Analysis (manual & auto) disabled. Set <code>VITE_OPENAI_API_KEY</code> in your .env.local file.
          </div>
            ...
            <p className="mt-3 text-sm text-gray-400 text-center">Click "Fetch ({selectedRssSourceUrls.length}) Feeds" or wait for auto-refresh.</p>
        ))}
         {selectedRssSourceUrls.length === 0 && !isLoadingRss && !isValidatingUrl && !isAutoRefreshing && !analysisBatchProgressData && !autoRefreshLog.length && (
            <p className="mt-3 text-sm text-orange-400 text-center">No RSS sources selected. Please select or add a source. Auto-refresh paused.</p>
         )}
      </div>
      ...
      <div className="mb-6 p-3 sm:p-4 bg-slate-800 bg-opacity-80 backdrop-blur-md rounded-xl shadow-2xl">
         <div className="flex flex-wrap gap-2 sm:gap-3 justify-center" role="tablist" aria-label="Data Views">
            ...
            <TabButton view={View.GEOJSON_OUTPUT} label="GeoJSON Output" icon={<GeoJsonIcon className="w-4 h-4 sm:w-5 sm:h-5"/>} disabled={!geoJsonDataState} />
        </div>
      </div>
      ...
      <footer className="mt-10 sm:mt-12 text-center text-xs sm:text-sm text-gray-500">
        <p>&copy; {new Date().getFullYear()} News Intelligence Dashboard. Powered by React, Tailwind CSS, and OpenAI API.</p>
      </footer>
    </div>
  );
};

export default AnalyzerAppComponent;
